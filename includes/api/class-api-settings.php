 ...
 * @subpackage Patterns_Lawyer/Patterns_Lawyer_Api_Settings
 */
 ...
 *
 * @see Patterns_Lawyer_Api
 */
if ( ! class_exists( 'Patterns_Lawyer_Api_Settings' ) ) {

	/**
	 * Patterns_Lawyer_Api_Settings
	 *
	 * @see WP_REST_Settings_Controller
	 * @package Patterns_Lawyer
	 * @since 1.0.0
	 */
	class Patterns_Lawyer_Api_Settings extends Patterns_Lawyer_Api {
		 ...
		 */
		public function run() {
			$this->type      = 'patterns_lawyer_api_settings';
		...
		public function get_item( $request ) {
			...
			$saved_options = patterns_lawyer_get_options();
		...
		public function update_item( $request ) {
			...
			if ( is_wp_error( rest_validate_value_from_schema( $params, $schema ) ) ) {
				return new WP_Error(
					...
					sprintf( __( 'The %s property has an invalid stored value, and cannot be updated to null.', 'patterns-lawyer' ), PATTERNS_LAWYER_OPTION_NAME ),
					array( 'status' => 500 )
				);
			}
			...
			/* Update options */
			patterns_lawyer_update_options(
				array(
			...
			/* Update user meta */
			patterns_lawyer_update_user_meta(
				get_current_user_id(),
				array(
		...
		public function get_settings_schema() {
			$setting_properties = apply_filters(
				'patterns_lawyer_options_properties',
				array(
...
/**
 * Return instance of  Patterns_Lawyer_Api_Settings class
 *
 * @since 1.0.0
 *
 * @return Patterns_Lawyer_Api_Settings
 */
function patterns_lawyer_api_settings() { //phpcs:ignore
	return Patterns_Lawyer_Api_Settings::get_instance();
}
patterns_lawyer_api_settings()->run();
