...
class PatternsLawyerSettingsApi {
	...
	bindEvents() {
		document.addEventListener( 'click', ( event ) => {
			...
			if ( target.closest( '.patterns-lawyer-gsn-close' ) ) {
				this.modifySettings( 'hide_get_started_notice', true );
			} else if (
				target.closest( '.patterns-lawyer-rn-permanent-close' )
			) {
				this.modifySettings( 'remove_review_notice_permanently', true );
			} else if (
				target.closest( '.patterns-lawyer-rn-temporary-close' )
			) {
				this.modifySettings(
				...
				const installBtn = target.closest(
					'.patterns-lawyer-install-plugins'
				);
				if ( installBtn ) {
					installBtn.classList.add( 'patterns-lawyer-processing' );
					this.installPlugins();
				}
			}
		} );
